...
> **Honest scoping (Google docs, 2026-06-05):** per "Using third-party SEO tools, services, and advice," no tool guarantees rankings and third-party tools have no access to Google's internal ranking data. seo-geo-skill's scores are **heuristics**, not Google-internal signals, so say so in reports, and validate GEO/AEO findings against Google's official guidance (Search Console is the first-party source).
...
```bash
# Quality score against the Quality Rater Guidelines heuristics
seogeo fetch https://example.com/post | seogeo content-quality --json
seogeo content-quality draft.md --threshold 70 --json

# Rewrite AI-typical phrasing into direct prose (deterministic 1:1 swaps)
seogeo content-humanize draft.md -o cleaned.md
seogeo content-humanize draft.md --json    # change log without writing

# Which claims have no citation near them?
seogeo content-verify draft.md --threshold 0.3 --json

# Site-reputation-abuse risk across a set of URLs
seogeo parasite-risk --urls-file urls.txt --json

# Expired-domain heritage check
seogeo domain-history example.com --topic "crypto signals" \
  --baseline-topic "veterinary services" --json
```

`content-quality` reports `filler_score`, `ai_pattern_score`,
`information_density`, `repetition_score`, and a composite
`overall_quality`, and exits non-zero below `--threshold`. Treat it as
advisory: no heuristic can determine authorship, and modern generation
passes every one of these checks. Pair it with `content-verify` — an
uncited-claim ratio is a much stronger signal of low-effort content than
any phrase list.

`content-humanize` only replaces phrases it knows. It never paraphrases
and never invents content, so its output is safe to diff.

`parasite-risk` scores by subfolder, not by site, because that is the unit
Google's site-reputation policy operates on. A `high` risk section needs a
human to check the actual contractual relationship — the scanner can only
see the patterns.
