...
```bash
seogeo hreflang https://example.com --json
seogeo hreflang --file page.html --json
```

The checker validates BCP 47 form and set consistency, and exits non-zero
on any error. It catches the mistakes that actually occur in the wild:

| Finding | Severity |
|---------|----------|
| `en-UK` instead of `en-GB` | error |
| `_` instead of `-` (`fr_FR`) | error |
| Region code in the language slot | error |
| Relative `href` | error |
| Duplicate `hreflang` value | error |
| Missing self-referencing entry | error |
| Missing `x-default` | warning |
| Lowercase region / uppercase language | warning |

The self-reference rule is the one people miss most: every page in an
hreflang set must list *itself* alongside its alternates, or the set is
ignored. A page-by-page check will not reveal this — you have to look at
whether each page's own URL appears in its own annotations, which is what
this command does.

`x-default` is a warning rather than an error because it is genuinely
optional, but recommend it whenever a site has more than two locales.
