...
2. **Moz API** (free signup): `seogeo backlinks-auth --check moz --json`
3. **Bing Webmaster** (free signup): `seogeo backlinks-auth --check bing --json`
...
Run `seogeo backlinks-auth --check --json` to detect all sources at once.
...
**Moz API:** `seogeo moz metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Common Crawl:** `seogeo commoncrawl <domain> --json` → PageRank, harmonic centrality, and low-confidence rank/presence data.
...
**Moz API:** `seogeo moz anchors <url> --json`

**Bing Webmaster:** `seogeo bing links <url> --json` (extract anchor text from link details)
...
**Moz API:** `seogeo moz domains <url> --json` → domains with DA scores

**Common Crawl:** `seogeo commoncrawl <domain> --json` → domain-level rank/presence data, no verified referring-domain counts
...
**Moz API:** Raw vendor spam_score from `seogeo moz metrics <url> --json` (source-label the value; apply thresholds only if verified against current Moz docs)

**Verification Crawler:** `seogeo verify-backlinks --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `seogeo moz pages <domain> --json`
...
**Bing Webmaster:** `seogeo bing compare <url1> <url2> --json`
...
**Moz API:** Compare DA/PA between domains via `seogeo moz metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `seogeo verify-backlinks --target <url> --links <file> --json`
